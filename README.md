...
GitHub Actions synchronizes provider variables only to Netlify's **production** context. Deploy Previews need their own provider credentials in the **Functions** scope; otherwise the expected result is `llm.skipped` with `missing_api_key`. Production secrets are not automatically copied to previews. The host requests the run configuration; round dialogue is requested by the host outside the tutorial. Loader facts are requested by the loading screens.
