...
const providerDefaults: Record<
  Exclude<DirectorProvider, "static">,
...
export function loadDirectorRuntimeConfig(
  environment: Environment,
): DirectorRuntimeConfig {
  const provider = parseProvider(
    environment.LLM_DIRECTOR_PROVIDER?.trim().toLowerCase(),
  );
  const staticConfiguration = parseStaticConfiguration(
    environment.LLM_DIRECTOR_STATIC_CONFIG,
  );
  ...
  return {
    ...
    baseUrl: withoutTrailingSlash(
      environment[`${prefix}_BASE_URL`]?.trim() || defaults.baseUrl,
    ),
