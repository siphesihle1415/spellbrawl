...
describe("gameReducer", () => {
  it("casts Firebolt after FIST then THRUST", () => {
    const primed = gameReducer(started(), { type: "GESTURE", playerId: "PLAYER_A", gesture: "FIST", at: 100 });
    const hit = gameReducer(primed, { type: "GESTURE", playerId: "PLAYER_A", gesture: "THRUST", at: 500 });
  ...
  it("requires a second player to point before breaking the Warden shield", () => {
    ...
    for (let index = 0; index < 3; index += 1) {
      ...
      state = gameReducer(state, { type: "GESTURE", playerId: "PLAYER_A", gesture: "THRUST", at: at + 200 });
    }
    ...
    state = gameReducer(state, { type: "GESTURE", playerId: "PLAYER_A", gesture: "THRUST", at: 7_200 });
  ...
  it("completes the Hexwyrm co-op sequence", () => {
    let state = started();
    let at = 100;
    const gesture = (playerId: "PLAYER_A" | "PLAYER_B", value: "FIST" | "THRUST" | "OPEN_PALM" | "POINT" | "PINCH" | "HANDS_APART") => {
      state = gameReducer(state, { type: "GESTURE", playerId, gesture: value, at });
      at += 100;
    };

    for (let hit = 0; hit < 3; hit += 1) { gesture("PLAYER_A", "FIST"); gesture("PLAYER_A", "THRUST"); }
    gesture("PLAYER_B", "POINT"); gesture("PLAYER_A", "FIST"); gesture("PLAYER_A", "THRUST");
    for (let hit = 0; hit < 4; hit += 1) { gesture("PLAYER_A", "FIST"); gesture("PLAYER_A", "THRUST"); }
    ...
    gesture("PLAYER_A", "POINT"); gesture("PLAYER_B", "PINCH");
    gesture("PLAYER_A", "FIST"); gesture("PLAYER_A", "THRUST");
    gesture("PLAYER_A", "FIST"); gesture("PLAYER_B", "HANDS_APART"); gesture("PLAYER_A", "THRUST");
